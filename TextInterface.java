 ...
 public static void main(String[] Args) throws IOException {
  ...
  while (!game.computerDefeated() && !game.userDefeated()) {
   if (player == Player.USER) {
    ...
    //Validate user input. The user must enter a letter A-J followed by a //number 1-10. The loop repeats until the user enters a valid value. 
    while (entry.length() < 2 || entry.length() >3 ||
    (entry.charAt(0) < 'A' || entry.charAt(0) > 'J') || (Integer.parseInt(entry.substring(1)) < 1 ||
   ...
   //The user has completed their turn, so we change the value of player //so that the next iteration of this loop will process a move by the //computer.
   player = Player.COMPUTER;
  } else {
   System.out.println("Computer's turn. Press any key to continue.");
   input.nextLine();
   //We call a Game function which returns a computer move. This allows us //to print that move for the user to see.
